<?php
	...
	class CSV{
		...
		public function upload(){
			if(!csrf_token(true)){
				echo $this->header();
				echo errorMsg("{$this->lang['csrf token expired or invalid']}<br>{$csv_file}{$this->error_back_link}");
				echo $this->footer();
			...
			if(!$csv_file || !is_readable($csv_file)){
				echo $this->header();
				echo errorMsg("{$this->lang['csv file upload error']}<br>{$csv_file}{$this->error_back_link}");
				echo $this->footer();
		...
		protected function get_table($silent = false){
			...
			if(!$table_ok){
				...
				echo $this->header();
				echo errorMsg(str_replace('<TABLENAME>', html_attr($table), $this->lang['table name title']) . ': ' . $this->lang['does not exist'] . $this->error_back_link);
				echo $this->footer();
				return false;
			}
...
/* ------------------------------------------------------ */
		...
		public function import(){
			...
			if($settings === false){
				$res['logs'][] = __LINE__ . ' ' . $this->lang['csv file upload error'];
				echo json_encode($res);
				return;
			}
		...
		protected function backup_table($start, $settings){
			...
			if($table === false) return array('error' => $this->lang['no table name provided']);
			...
			if(!sql("create table if not exists `{$btn}` like `{$stable}`", $eo))
				return array('error' => str_replace('<TABLE>', $table, $this->lang['error backing up table']));
		...
		protected function get_query(&$csv_data, $settings, &$query_info){
			...
			$table = $this->get_table(true);
			if($table === false){
				$query_info['error'] = $this->lang['no table name provided'];
				return false;
			}
			$stable = makeSafe($table);
			...
			if(!isset($settings['mappings']) || !is_array($settings['mappings']) || !count($settings['mappings'])){
				$query_info['error'] = $this->lang['error reading csv data'];
				return false;
			}
		...
		protected function start($new_start = false){
			...
			if(!$csv){
				...
				return array('error' => __LINE__ . ' ' . $this->lang['csv file upload error']);
			}
		...
		public function show_import_progress(){
			echo $this->header();
			if(!csrf_token(true)){
				echo errorMsg("{$this->lang['csrf token expired or invalid']}<br>{$this->error_back_link}");
				echo $this->footer();
				return;
			}
			?>
